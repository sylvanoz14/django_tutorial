...
def generate_report(results, top_n=5):
    ...
    lines.append('=' * 68)
    lines.append('')
    lines.append('PRIMARY RECOMMENDATION: TRI-SUBURB CENTRE SITE')
    lines.append('(Serving Mambourin, Wyndham Vale & Manor Lakes from one location)')
    lines.append('-' * 68)
    lines.append('')
    lines.append('IMPORTANT: the automated per-suburb ranking below scores each suburb')
    lines.append('independently and cannot express "one site serving three suburbs at')
    lines.append('once." For that specific goal, the recommendation is:')
    lines.append('')
    lines.append('  >>> BALLAN ROAD CORRIDOR, WYNDHAM VALE (near Manor Lakes Central) <<<')
    lines.append('')
    lines.append('  Ballan Road is the arterial that physically connects Wyndham Vale')
    lines.append('  and Manor Lakes, and sits almost exactly on the geometric centre of')
    lines.append('  Mambourin / Wyndham Vale / Manor Lakes (approx -37.886, 144.599).')
    lines.append('  It is also an active, real commercial corridor:')
    lines.append('')
    lines.append('  - Manor Lakes Central, 455 Ballan Rd: established shopping centre')
    lines.append('    (Coles, Kmart, Best & Less, Anytime Fitness, ~20,000 sqm GLA).')
    lines.append('    Colliers currently lists retail space for lease here. Co-locating')
    lines.append('    adjacent to this anchor captures existing family foot traffic from')
    lines.append('    all three suburbs at once. [UNVERIFIED — detail page blocked')
    lines.append('    automated retrieval; confirm current tenancy size with the agent]')
    lines.append('  - 440 & 462 Ballan Rd / 2 Hirata Blvd: 2.15 ha, approved for up to')
    lines.append('    11,000 sqm large-format retail, directly opposite Manor Lakes')
    lines.append('    Central. SOLD off-market (settlement Aug 2025) — NOT available,')
    lines.append('    but confirms developers are already targeting this exact corridor.')
    lines.append('  - 819 Ballan Rd, Manor Lakes: 45.23 ha vacant land nearby — likely')
    lines.append('    a master-planned parcel; worth asking whether a smaller lot can')
    lines.append('    be carved out.')
    lines.append('')
    lines.append('  NOTE ON TRUGANINA/TARNEIT: these still score highest in the raw')
    lines.append('  per-suburb model below, largely on verified warehouse availability.')
    lines.append('  But they sit outside the Mambourin/Wyndham Vale/Manor Lakes pocket')
    lines.append('  entirely (10-20 km away) and do not serve that catchment — and per')
    lines.append('  local knowledge, Crocs Playcentre in Hoppers Crossing already')
    lines.append('  covers the Truganina/Palmers Rd corridor, which the model')
    lines.append('  under-weighted before this competitor was added. If your goal is')
    lines.append('  specifically to serve Mambourin + Wyndham Vale + Manor Lakes, the')
    lines.append('  Ballan Road corridor above is the right answer, not Truganina.')
    lines.append('')
    lines.append('=' * 68)
    lines.append('')
    lines.append('TOP {} RECOMMENDED SUBURBS (AUTOMATED PER-SUBURB MODEL)'.format(top_n))
    lines.append('-' * 68)
    lines.append('')
